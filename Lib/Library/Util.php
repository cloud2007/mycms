...
class Util {
    ...
    public function getpicthumb($pic,$type){
        if( is_file( ROOTPATH . str_replace('.', '_'.$type.'.',$pic)) ) return 'http://img.haoloubang.com/' . str_replace('/uploadfiles/' , '' , str_replace('.', '_'.$type.'.',$pic));
		if( is_file( ROOTPATH . $pic ) ) return 'http://img.haoloubang.com/' . str_replace('/uploadfiles/' , '' , $pic);
	...
	*/
	...
	public function switch_val_en_top($number){//传入编码GB2312编码的中文值，返回对应的拼音“首字母”
		$en_head = array(
			array(-20319,-20284,'A'),
			array(-20283,-19776,'B'),
			array(-19775,-19219,'C'),
